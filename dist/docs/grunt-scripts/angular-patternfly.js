...
;angular.module('patternfly.navigation').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('navigation/application-launcher.html',
    "<div><div class=\"applauncher-pf dropdown dropdown-kebab-pf\" ng-class=\"{'applauncher-pf-block-list': !isList()}\" uib-dropdown keyboard-nav=true is-open=isOpen on-toggle=toggled(open)><a id=domain-switcher-{{$id}} class=\"dropdown-toggle drawer-pf-trigger-icon\" uib-dropdown-toggle ng-class=\"{'disabled': isDisabled() || !items().length}\" href><span class=\"fa fa-th applauncher-pf-icon\"></span> <span class=applauncher-pf-title>Application Launcher <i class=caret aria-hidden=true></i></span></a><ul class=\"dropdown-menu dropdown-menu-right\" uib-dropdown-menu role=menu aria-labelledby=domain-switcher-{{$id}}><li title={{item.tooltip}} class=applauncher-pf-entry role=menuitem ng-repeat=\"item in items()\"><a class=applauncher-pf-link ng-href={{item.href}} target=_blank><i class=\"applauncher-pf-link-icon fa fa-2x\" ng-class=item.iconClass ng-if=!hiddenIcons() aria-hidden=true></i> <span class=applauncher-pf-link-title>{{item.title}}</span></a></li></ul></div></div>"
  );

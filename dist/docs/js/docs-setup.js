NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "keywords": "$interval $scope alert amount angular api arc availchart availconfig availdata availlabel blue c3 calculate calculated camelcase center center-label centerlabelfn change chart chart-height chartctrl charterr chartheight chartid chartnothresh chartok charts chartwarn checkbox checkbox-inline class click clicked clockwise col-md-12 col-md-3 color colors component config configdynamic configerr confignothresh configuration configwarn container-fluid contents controller custchart custchartheight custconfig custdata customize data dataavailable datadynamic dataerr datanothresh datawarn default defined dependent determine directive display displays donut donut-tooltip-pf empty error example exist external fill flag form form-group format full function gb gray grey handle handling height html http indicator js label labelconfig labeldynamic labelfn left legend list list-style-type mhz module moves ng-controller ng-model nolabel nonechart noneconfig nonedata nowrap number object on-threshold-change onclickfn options orange org orientation patternfly pctchart pctconfig pctdata pctlabel percent percentage portion prefix properties radial red rendered rendering represented representing return role row script set size specifies starts string style surpassed text text-center text-left text-right three threshlabel threshold thresholdchanged thresholds thresolds title tool tooltip tooltipfn total true type undefined unique unit units usage usedchart usedconfig useddata usedlabel user values warning white-space width wording"
    },
    {
      "section": "api",
